  ...
  return (
    <div className="flex flex-col gap-4 w-full max-w-md mx-auto">
      ...
      <div className="rounded-md border border-border overflow-hidden">
        ...
        <div className="px-4 py-3 bg-surface-2 border-b border-border flex items-center justify-between gap-4">
          <div>
            <p className="text-xs tracking-[0.16em] uppercase text-amber/80">
              ...
              return (
                <React.Fragment key={head.date}>
                  ...
                  <tr
                    ...
                    className={`
                      ${!isLast || isExpanded ? "border-b border-border/60" : ""}
                      ${head.isStartDate ? "bg-amber/10" : ""}
                      ${hasIntermediate ? "cursor-pointer hover:bg-surface-2/40 transition-colors duration-100" : ""}
                    `}
                  >
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <span
                          className={`${head.isStartDate ? "text-amber-500/80" : "text-text-primary"}`}
                        >
                          {formatDate(head.date)}
                        </span>
                        {/* Badge shown only on the contract start row */}
                        {head.isStartDate && (
                          <span className="text-xs tracking-[0.1em] uppercase bg-amber/20 text-amber/80 px-1.5 py-0.5 rounded">
                            inicio
                          </span>
                        )}
                      </div>
                    ...
                    <td
                      className={`px-4 py-3 text-right font-medium ${head.isStartDate ? "text-amber-400/80" : "text-text-primary"}`}
                    >
                      {shortAmt}
                    </td>
                    ...
                    <td className="pr-3 text-right">
                      <span
                        className={`text-sm text-gray-600 select-none inline-block transition-transform duration-150
                  ...
                  {hasIntermediate &&
                    ...
                    intermediates.map((rec, j) => {
                      ...
                      return (
                        <tr
                          key={rec.date}
                          className={`bg-surface-2/60 ${!isIntLast ? "border-b border-border/30" : "border-b border-border/60"}`}
                        >
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="rounded-md border border-border bg-surface-2 p-3 flex flex-col gap-1">
          <ResultLabel label="Inicial" />
          <p className="text-sm font-medium text-gray-400">{shortStart}</p>
        </div>
        <div className="rounded-md border border-border bg-gray-900 p-3 flex flex-col gap-1">
          <ResultLabel label="final" />
          <p className="text-sm font-medium text-text-primary">{shortFinal}</p>
        </div>
        {/* Total variation card: highlighted in amber to draw attention */}
        <div className="rounded-md border border-amber/20 bg-amber/5 p-3 flex flex-col gap-1">
          ...
          {shortIncrease !== fullIncrease && (
            <p className="text-xs text-gray-600">{fullIncrease}</p>
          )}
        </div>
      </div>
    </div>
